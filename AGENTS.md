...
- `pnpm lint` may report a pre-existing lint error in `apps/mobile/src/hooks/use-color-scheme.web.ts` (`react-hooks/set-state-in-effect`). This is existing application code, not an environment issue.
- `npx tsc --noEmit` reports an error for the side-effect import of `@/global.css` in `apps/mobile/src/constants/theme.ts` (and any other CSS imports). These are resolved by the Expo/Metro bundler at runtime, not by `tsc`, so they are expected and do not affect the running app.
