...
**Decision:** Separate `User` (account) from `Profile` (1:1); put domain validation in `packages/shared/domain` (`@repo/domain`); use Supabase Auth as credential source with a thin `public.users` row for app status. Discovery requires both account and profile `active`. Matching/swipes/messages are out of scope for this pass.

Persistence ownership (Drizzle as schema source of truth, shared PKs, RLS) is recorded in [0002-drizzle-as-schema-source-of-truth.md](./0002-drizzle-as-schema-source-of-truth.md).
