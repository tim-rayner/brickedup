...
- **Packages**: Shared packages (`theme`, `eslint-config`, `typescript-config`, `shared/domain`, `shared/db`) have no separate contexts. Theme or design vocabulary that becomes contested product language belongs in the root context. Drizzle tables in `packages/shared/db` and Zod in `packages/shared/domain` implement root glossary terms — they do not invent competing names.
